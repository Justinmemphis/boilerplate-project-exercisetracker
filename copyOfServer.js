// This is the server.js file that uses a String type for the Date field - saved on 05/25/22
...
app.post('/api/users/:_id/exercises', async (req, res) => {
  ...
  if (!date) { 
    ...
    let newText = newToday.toDateString()
    console.log(newText)
    date = newText
  } else {
    let secondDate = new Date(req.body.date)
    let secondText = secondDate.toDateString()
  ...
  try {
    ...
    */
      ...
      if (req.query['from']) {
        ...
        from = new Date("1970-01-01").toDateString() 
        numFrom = Date.parse(from)
      }
      ...
      if (req.query['to']) {
        ...
        to = new Date("2022-01-01").toDateString()
        numTo = Date.parse(to)
